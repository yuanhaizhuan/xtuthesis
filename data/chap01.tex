...
这是 \xtuthesis\cite{xtuthesis} 的示例文档，基本上覆盖了模板中所有格式的设置。建
议大家在使用模板之前，除了阅读《\xtuthesis{} 用户手册》，这个示例文档也最好能看一
看。
...
\begin{table}[htb]
  ...
  \label{tab:template-files}
    \begin{tabularx}{\linewidth}{lX}
      ...
      xtuthesis.ins & \LaTeX{} 安装文件，DocStrip\footnote{表格中的脚注} \\
      xtuthesis.dtx & 所有的一切都在这里面\footnote{再来一个}。\\
      xtuthesis.cls & 模板类文件。\\
      xtuthesis-numeric.bst    & 参考文献 BibTeX 样式文件。\\
      xtuthesis-author-year.bst    & 参考文献 BibTeX 样式文件。\\
      xtusetup.tex   & 基本配置和常用的包、命令，减轻主文件的负担。\\
      \bottomrule[1.5pt]
    \end{tabularx}
  \end{minipage}
\end{table}
...
\begin{table}[htbp]
  ...
  \begin{minipage}[t]{0.8\textwidth}
    ...
    \end{tabularx}\\[2pt]
    \footnotesize 注：数据来源《\xtuthesis{} 使用手册》。\\
    *：东部\\
    **：西部
  \end{minipage}
\end{table}
...
本模板推荐使用 BibTeX，分别提供数字引用（\texttt{xtuthesis-numeric.bst}）和作
者年份引用（\texttt{xtuthesis-author-year.bst}）样式，基本符合学校的参考文献格式
